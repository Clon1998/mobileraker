...
void main() {
  ...
  test('Test klippyService init', () {
    ...
    when(mockRpc.addMethodListener(any, 'notify_klippy_ready'))
        .thenReturn(null);

    when(mockRpc.addMethodListener(any, 'notify_klippy_shutdown'))
        .thenReturn(null);

    when(mockRpc.addMethodListener(any, 'notify_klippy_disconnected'))
        .thenReturn(null);
    ...
    when(mockRpc.sendJRpcMethod('server.info')).thenAnswer(
            ...
            RpcResponse.fromJson(jsonDecode(serverInfoJson)));
    ...
    when(mockRpc.sendJRpcMethod('printer.info')).thenAnswer(
            (realInvocation) async =>
            RpcResponse.fromJson(jsonDecode(printerInfoJson)));
