...
import 'package:common/data/dto/config/config_gcode_macro_param.dart';
...
void main() {
  ...
  ''';
    ...
    var macroParam = configGcode.params['USE_HEATER'];
    expect(macroParam, isInstanceOf<ConfigGcodeMacroParam>());
    // expect(macroParam?.defaultValue, "False");
    // expect(macroParam?.type, "bool");
  });

  test('ConfigGcodeMacro.fromJson() creates ConfigGCode instance from JSON and can read params', () {
    const jsonString = '''
    {
  ...
  ''';
    ...
    final jsonMap = json.decode(jsonString);
    final configGcode = ConfigGcodeMacro.fromJson('_set_sb_leds', jsonMap);

    expect(configGcode.description, 'G-Code macro');
    expect(configGcode.macroName, '_set_sb_leds');
    expect(configGcode.gcode,
        "\n{% set red = params.RED|default(0)|float %}\n{% set green = params.GREEN|default(0)|float %}\n{% set blue = params.BLUE|default(0)|float %}\n{% set white = params.WHITE|default(0)|float %}\n{% set led = params.LED|string %}\n{% set idx = (params.IDX|string).split(',') %}\n{% set transmit_last = params.TRANSMIT|default(1) %}\n\n{% for led_index in idx %}\n{% set transmit=transmit_last if loop.last else 0 %}\nset_led led={led} red={red} green={green} blue={blue} white={white} index={led_index} transmit={transmit}\n{% endfor %}");
    expect(configGcode.params.length, 7);
    expect(configGcode.params.keys, containsAll(['RED', 'GREEN', 'BLUE', 'WHITE', 'LED', 'IDX', 'TRANSMIT']));

    void expectParam(String name, String? type, String? defaultValue) {
      var macroParam = configGcode.params[name];
      expect(macroParam, isInstanceOf<ConfigGcodeMacroParam>());
      expect(macroParam?.defaultValue, defaultValue);
      expect(macroParam?.type, type);
    }

    expectParam('RED', 'float', '0');
    expectParam('GREEN', 'float', '0');
    expectParam('BLUE', 'float', '0');
    expectParam('WHITE', 'float', '0');
    expectParam('LED', 'string', null);
    expectParam('IDX', 'string', null);
  });
}
